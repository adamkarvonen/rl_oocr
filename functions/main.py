...
def compute_metrics(model_out_list, top_logprobs_list, target, func_outs=None):
    ...
    for model_out, top_logprobs in zip(model_out_list, top_logprobs_list):
        if func_outs is not None and (
            func_outs["name"] == "latent_func_multiple_choice"
            or func_outs["name"] == "func_values"
        ):
            target_logprob = np.nan
            l2_distance = np.nan

            incorrect_letters = [
                letter for letter in string.ascii_uppercase if not letter in target
            ]
            ...
            func_outs is not None
            and func_outs["name"] == "function_definition_freeform"
        ):
            target_logprob = np.nan
            l2_distance = np.nan

            raise Exception(
                "Only run free form definition eval in a sandbox environment, this executes model outputs"
            )
            ...
            if np.isnan(prob):
                ...
                if len(model_out_list) <= 1:
                    raise ValueError(
                        "invalid setting detected!!! target: {}, model_out: {}".format(
                            target, model_out
                        )
                    )
                prob = float(correct)
...
def log_metrics(config, metrics, datapoint, model_out, top_logprobs):
    ...
    datapoint_metrics = compute_metrics(
        model_out, top_logprobs, datapoint["target"], func_outs=datapoint["func_outs"]
    )
    ...
    if "var_dict" in dataset_config:
        ...
        with open(functions_path, "w") as file:
            ...
            for key, value in dataset_config.var_dict.items():
                ...
                if type(value) == str:
                    file.write(
                        f"{key} = eval(function_definitions['{value}']['python_definition'])\n"
                    )
...
def dataset_eval(config, client, dataset):
    ...
    for i in tqdm(range(len(dataset))):
        ...
        top_logprobs = [
            [
                {item.token: item.logprob for item in cont.top_logprobs}
    ...
    print(
        "Correct: {:.2f} ± {:.2f}".format(
            np.mean(metrics["correct"]), sem(metrics["correct"])
        )
    )
    ...
    if config.task == "eval":
        ...
        if not path.exists(path.join(config.exp_path, "test_config.yaml")):
            ...
            if "train_functions" in config.dataset:
                test_config.dataset.train_functions = deepcopy(
                    config.dataset.train_functions
                )

            if "test_functions" in config.dataset:
                test_config.dataset.test_functions = deepcopy(
                    config.dataset.test_functions
                )
        ...
        write_to_json(
            train_set,
            config.exp_path,
            "{}_train_dataset.json".format(config.finetune.suffix),
        )
        write_to_json(
            test_set,
            config.exp_path,
            "{}_test_dataset.json".format(config.finetune.suffix),
        )
        ...
        if path.exists(path.join(config.exp_path, "finetune_response.json")):
            raise FileExistsError(
                "A fine-tuning job has already been created for this experiment"
            )
        ...
        assert path.exists(train_data_path), (
        ...
        #     file=open(
        #         "dev/047_functions/finetune_01/047_func_01_train_oai_small.jsonl", "rb"
        #     ),
        #     purpose="fine-tune",
        # )
        ...
        #     file=open(
        #         "dev/047_functions/finetune_01/047_func_01_test_oai_small.jsonl", "rb"
        #     ),
        #     purpose="fine-tune",
        # )
        ...
        # upload files
        # train_file_api = client.files.create(
        ...
        train_file_api = FileResponse("file-VGSnSVJuaTCVFiPrQNjt2K")

        test_file_api = client.files.create(
            file=open(test_data_path, "rb"), purpose="fine-tune"
        )
        ...
        write_to_json(
            recursive_obj_to_dict(response), config.exp_path, "finetune_response.json"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_path", type=str, required=True, help="Path to experiment folder"
    )
    parser.add_argument(
        "--config", type=str, default=CONFIG_NAME, help="name of config file"
    )
    parser.add_argument(
        "--task",
        type=str,
        required=True,
        choices=["eval", "prepare_finetune", "finetune"],
    ...
    parser.add_argument(
        ...
        type=str,
        default=KEYS,
        help="path of file with OpenAI keys. Right now "
