...
def recursive_obj_to_dict(obj):
    ...
    if isinstance(obj, (int, float, str, bool)):
        ...
        return {
            key: recursive_obj_to_dict(value) for key, value in obj.__dict__.items()
        }
        ...
        return str(
            obj
        )  # Convert types that are not directly serializable to string or implement custom handling
...
def calculate_log_probability(token_dicts, target, model_out):
    ...
    if len(target_sequence) > 1:
        ...
        if len(token_dicts) == 1 and target_sequence[0] in token_dicts[0]:
            ...
            baseline_logprob = np.log(1.0 / enc.n_vocab)
...
def get_letter_list(keys_list, rng, size=100):
    if "X1" in keys_list:
        ...
        letter_list = [
            "".join(rng.choice(list(string.ascii_uppercase), size=length))
            for _ in range(size)
        ]
